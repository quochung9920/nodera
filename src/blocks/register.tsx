...
function LegacyNotice() {
	return <Notice status="warning" isDismissible={false}>{__('Legacy Nodera block. WordPress 7.1 now provides native Core Accordion/Tabs; new Nodera AI output uses Core blocks.', 'nodera')}</Notice>;
}

function AccordionEdit({ attributes, setAttributes }: any) {
	return <div className="nodera-accordion-editor"><LegacyNotice/><TextControl label={__('Accordion title', 'nodera')} value={attributes.title || ''} onChange={(title)=>setAttributes({title})} /><RichText tagName="div" value={attributes.content || ''} onChange={(content)=>setAttributes({content})} placeholder={__('Accordion content…', 'nodera')} /></div>;
}
